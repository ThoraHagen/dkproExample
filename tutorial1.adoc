...
def main(input_folder, output_folder):
    ...
    # If you want to create a circular graph instead of a regular one, 
    # place '#' in front of every line of the following block and 
    # erase the three '#' after 'Circular drawing', then run the script (again)
...
Discussion
++++++++++
In this recipe we created a visualization of an author's social network using the output of the DARIAH-DKPro-Wrapper and two simple Python scripts to gain knowledge of the authors' relations. This is a good starting point for further research, as the existing 
relations can now be investigated more closely. The edges could even hint at possible connections that have not yet been made.
This kind of analysis therefore presents a great opportunity or groundwork, because this tool might structure the information differently then originally assumed. [Add some more]
